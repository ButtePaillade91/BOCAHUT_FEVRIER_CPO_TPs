...
public class Grille {
    ...
    public void tasserColonne(int colonneàtasser) {
        /*for (int i=0 ; i<6 ; i++) {
            if (i == 5) {
                CellulesJeu[i][colonneàtasser].jetonCourant = null;
            }
            else {
                if (CellulesJeu[i][colonneàtasser].jetonCourant == null) {
                    CellulesJeu[i][colonneàtasser].jetonCourant = CellulesJeu[i+1][colonneàtasser].jetonCourant;
                    CellulesJeu[i+1][colonneàtasser].jetonCourant = null;
                }
            }
        }*/
        int vérif=0;
        ...
        /*
        int i=0;
        while(CellulesJeu[i][colonneàtasser].jetonCourant!=null) {
            i++;
        }
        while(i<5) {
            CellulesJeu[i][colonneàtasser].jetonCourant=CellulesJeu[i+1][colonneàtasser].jetonCourant;
        ...
        CellulesJeu[5][colonneàtasser].jetonCourant=null;
        */
    }
    ...
    public Jeton recupererJeton(int lignedujeton, int colonnedujeton) {
        CellulesJeu[lignedujeton][colonnedujeton].jetonCourant=null;
        return(CellulesJeu[lignedujeton][colonnedujeton].recupererJeton());
    }
    public boolean placerDesintegrateur(int lignedesintegrateur, int colonnedesintegrateur) {
        if(CellulesJeu[lignedesintegrateur][colonnedesintegrateur].presenceDesintegrateur()==false) {
            CellulesJeu[lignedesintegrateur][colonnedesintegrateur].placerDesintegrateur();
            return(true);
        }
        else {
            return(false);
    ...
    public boolean supprimerJeton(int lignejetonàdésintégrer, int colonnejetonàdésintégrer) {
        if(CellulesJeu[lignejetonàdésintégrer][colonnejetonàdésintégrer].jetonCourant!=null) {
            CellulesJeu[lignejetonàdésintégrer][colonnejetonàdésintégrer].jetonCourant=null;
            return(true);
        }
        else {
            return(false);
